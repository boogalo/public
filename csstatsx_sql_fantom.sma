...
public plugin_precache()
{
	...
	* хост mysql
	*/
	cvar[CVAR_SQL_HOST] = register_cvar("csstats_sql_host","127.0.0.1",FCVAR_UNLOGGED|FCVAR_PROTECTED)
	...
	*/
	cvar[CVAR_SQL_DB] = register_cvar("csstats_sql_db","freeman",FCVAR_UNLOGGED|FCVAR_PROTECTED)
	...
	*/
	cvar[CVAR_SQL_TYPE] = register_cvar("csstats_sql_type","sqlite")
	...
	*/
	...
	if(!cvar[CVAR_RANKBOTS])
		cvar[CVAR_RANKBOTS] = register_cvar("csstats_rankbots","0")
	...
	*/
	cvar[CVAR_ASSISTHP] = register_cvar("csstats_sql_assisthp","0")
...
#endif
{
	...
	// F@nt0M
	FW_PlayerLoaded = CreateMultiForward("csxsql_player_loaded", ET_IGNORE, FP_CELL, FP_CELL, FP_CELL)
	FW_RankUpdated = CreateMultiForward("csxsql_rank_updated", ET_IGNORE, FP_CELL, FP_CELL, FP_CELL)
...
public plugin_end()
{
	...
	DestroyForward(FW_PlayerLoaded)
	DestroyForward(FW_RankUpdated)
}
...
public SQL_Handler(failstate,Handle:sqlQue,err[],errNum,data[],dataSize){
	...
	switch(data[0])
	{
		...
		case SQL_LOAD: // загрзука статистики игрока
		{
			...
			ExecuteForward(FW_PlayerLoaded, dummy_ret, id, player_data[id][PLAYER_ID], player_data[id][PLAYER_RANK])
		}
			...
			while(SQL_MoreResults(sqlQue))
			{
				...
				for(new i ; i < MAX_PLAYERS ; i++)
				{
					if(player_data[i][PLAYER_ID] == pK)	// задаем ранк по первичному ключу
					{
						...
						ExecuteForward(FW_RankUpdated, dummy_ret, i, player_data[i][PLAYER_ID], player_data[i][PLAYER_RANK])
					}
				}
